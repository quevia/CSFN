...
from tag_model.modeling import TagEmebedding, TagPooler, BiGRU
...
class BertForSequenceClassificationTag(BertPreTrainedModel):
    def __init__(self, config, num_labels=1, tag_config=None):
        ...
        use_tag = True
        if use_tag:
            self.pool = nn.Linear(tag_config.hidden_size,tag_config.hidden_size)
            self.classifier = nn.Linear(config.hidden_size , tag_config.hidden_size)
            #self.classifier2 = nn.Linear(tag_config.hidden_size , 1)
            self.classifier1 = nn.Linear(config.hidden_size , 1)
            self.classifier2 = nn.Linear(tag_config.hidden_size , 1)
            self.classifier3 = nn.Linear(tag_config.hidden_size+config.hidden_size , 1)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, input_tag_ids=None, labels=None,task_id=None):
        input_ids = input_ids.view(-1, input_ids.size(-1))
        token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1))
        attention_mask = attention_mask.view(-1, attention_mask.size(-1))
        #input_tag_ids = input_tag_ids.view(-1, input_tag_ids.size(-1))
        sequence_output,bert_pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        no_cuda = False
        context_feature = self.classifier(bert_pooled_output)
        flat_input_tag_ids = input_tag_ids.view(-1,input_tag_ids.size(-1))
        tag_output = self.tag_model(flat_input_tag_ids)
        # combine process
        a1 = context_feature.view(context_feature.size(0),-1,context_feature.size(1))
        weight = tag_output.bmm(torch.transpose(a1,1,2))
        soft_weight = torch.nn.functional.softmax(weight,1)
        b1 = tag_output * soft_weight  # the enhanced semantic representations with contextual information
        tag_pooled_output = torch.sum(b1,dim=1)
        #first_token_tensor = b1[:, 0]
        #tag_pooled_output = self.pool(first_token_tensor)
        tag_pooled_output = self.activation(tag_pooled_output)
        logits1 = torch.cat([bert_pooled_output,tag_pooled_output],-1)
        logits = self.classifier3(logits1)
        '''
        logits1 = self.classifier1(bert_pooled_output)
        logits2 = self.classifier2(tag_pooled_output)
        logits=0.5*logits1 + 0.5*logits2
        '''
        logits = logits.view(-1, self.num_choice)
