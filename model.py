...
#import pytorch_transformers.tokenization_roberta as tokenization
...
def convert_features_to_tensors(features, output_mode, is_multi_choice=True):
    ...
    if is_multi_choice:
        ...
        for f in features:
            ...
            input_mask.append([])
            segment_ids.append([])
            input_tag_ids.append([])
            for i in range(n_class):
                input_ids[-1].append(f[i].input_ids)
                input_mask[-1].append(f[i].input_mask)
                segment_ids[-1].append(f[i].segment_ids)
                input_tag_ids[-1].append(f[i].input_tag_ids)
        ...
        for f in features:
            ...
            input_tag_ids.append(f.input_tag_ids)
            label_id.append(f.label_id)

    all_input_ids = torch.tensor(input_ids, dtype=torch.long)
    all_input_mask = torch.tensor(input_mask, dtype=torch.long)
    all_segment_ids = torch.tensor(segment_ids, dtype=torch.long)
    all_input_tag_ids = torch.tensor(input_tag_ids, dtype=torch.long)

    if output_mode in ["classification", "multi-choice"]:
        all_label_ids = torch.tensor(label_id, dtype=torch.long)
    elif output_mode == "regression":
        all_label_ids = torch.tensor(label_id, dtype=torch.float)
    data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_input_tag_ids, all_label_ids)
    return data
...
def train(args, train_datasets, model, tokenizer):
    ...
    for epoch, _ in enumerate(train_iterator):
        ...
        for step, _ in enumerate(epoch_iterator):
            ...
            input_ids, input_mask, segment_ids, input_tag_ids, label_ids = batch
            outputs = model(input_ids, segment_ids, input_mask, input_tag_ids,  label_ids, task_id)
            loss = outputs[0]
...
def load_and_cache_examples(args, task, tokenizer, set_type='train'):
    ...
    cached_features_file = os.path.join(args.data_dir[task], 'cached_{}'.format(set_type))
...
def main():
    ...
    # tokenizer = tokenization.RobertaTokenizer(vocab_file=os.path.join(args.bert_model, 'vocab.json'),
    #                                merges_file=os.path.join(args.bert_model, 'merges.txt'))
    ...
    tag_config = TagConfig(tag_vocab_size=vocab_size,
                           ...
                           output_dim=10,
                           dropout_prob=0.1)
    model = BertForSequenceClassificationTag.from_pretrained(args.bert_model,tag_config=tag_config)
