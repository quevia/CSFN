...
def convert_examples_to_features(examples, label_list, max_seq_length,tokenizer,tag_tokenizer, is_multi_choice=True):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        token_b = example.text_b   # question + option
        ...
        print('tokens_a_tags',tokens_a_tags)
        ...
        _truncate_seq_pair(tokens_a, tag_ids_list_a, max_seq_length - 2)
        ...
        input_tag_ids = input_que_tag_ids
        ...
        if is_multi_choice:
            ...
            features.append(
                InputFeatures(
                    ...
                    label_id=label_id)) 
    return features
def _truncate_seq_pair(tokens_a, tag_ids_list_a, max_length):
    while True:
...
train_examples = cola.get_train_examples(data_dir)
train_features = convert_examples_to_features(train_examples, labels, max_seq_length,tokenizer,tag_tokenizer, is_multi_choice=False)
print("Saving features into cached file ",train_f)
torch.save(train_features, train_f)
'''
test_examples = rte.get_test_examples(data_dir)
test_features = convert_examples_to_features(test_examples, labels, max_seq_length,tokenizer,tag_tokenizer, is_multi_choice=False)
print("Saving features into cached file ", test_f)
torch.save(test_features, test_f)
'''
dev_examples = cola.get_dev_examples(data_dir)
dev_features = convert_examples_to_features(dev_examples, labels, max_seq_length,tokenizer,tag_tokenizer, is_multi_choice=False)
